...
const List = ({datas,deleteTodo,updateTodo,markTodo})=>{
  
  return (
    <div >
        <ul>
            {datas.map((data)=><Event data={data} deleteTodo={deleteTodo} updateTodo={updateTodo} markTodo={markTodo} key={data.id}/>)}
        </ul>
    </div>
  );
}
